<template>
    <form @submit.prevent="submit" class="rounded border bg-white">
        <div v-show="form.media" class="px-4 pt-3 inline-block relative">
            <img :src="form.media" class="max-w-full rounded border border-gray-200" style="max-height: 400px;">
        ...
        <textarea
            ...
            class="block w-full rounded appearance-none leading-tight px-4 py-3 resize-none focus:outline-none"
            rows="4"></textarea>
        ...
        <div class="flex items-end justify-between px-4 py-3">
            <upload
                ...
                <button @click.prevent="toggle" class="text-gray-600 hover:text-gray-800 focus:outline-none">
                    <span v-if="uploading" class="text-sm text-gray-700">Uploading ...</span>
            ...
            <div>
                <button
                    class="px-3 py-2 rounded text-gray-700 text-sm font-medium focus:outline-none"
                    @click.prevent="$emit('cancelled')"
                    type="button"
                >Cancel</button>
                <button
                    class="px-3 py-2 rounded bg-blue-500 text-white text-sm font-medium focus:outline-none focus:shadow-outline"
...
export default {
    ...
    methods: {
        submit () {
            axios.post(`/comments/${this.comment.id}/replies`, this.form)
